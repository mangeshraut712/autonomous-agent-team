...
Important: write only to the shared workspace `../../intel/` paths above.
Do not write outputs to `agents/dwight/intel/`.
...
## Fallback Behavior (Tool Outage)
- If live-web tools are unavailable (for example missing provider API keys or browser/web tool outage):
  - Do not delegate the task away; complete the run directly.
  - If `PARALLEL_API_KEY` is available, run:
    `../../scripts/parallel-search.sh --mode agentic --max-results 10 --objective "<your objective>" --format markdown`
  - Use returned source URLs/excerpts as citations in intel output.
  - If Parallel helper is unavailable, produce a **Local-Only Intel Update** using existing files (`intel/`, `memory/`, tracked sources from prior runs).
  - Clearly mark the report as `[LIMITED: LIVE SOURCES UNAVAILABLE]`.
  - Never invent or backfill fake fresh sources.
  - End with a single action line listing what access is needed to restore full sweep capability.
